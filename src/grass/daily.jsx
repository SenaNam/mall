...
- 사용자 프론트 사업자 번호로 강제로 번호 로그인 되도록 -> 김부장님이 만들어 주실거임 (내부망, 외부망 다?)
- IBK로그인 화면 스피너 아래 + 로그인 컴포넌트 위로 opacity 색상 넣어달라함
- [유져 로그인 로직 수정] cmi002DB has00, has04 체크에서 cmi001에서 JNNG_YN, SPAGN_YN 체크로 변경 
- [어드민] > 권한관리 > X권한, T권한
    - 화면에 권한현황 목록에 출력
    - 화면에 이력 죄회 목록에도 출력
    - 권한 등급 필터 조회
    - 신규 등록에서 권한등급 S만 X,T 주기
- [페이고스,디수정] 개인정보 수집 및 이용 동의서는 담당자 입력 화면에서 체크
- [유져] API연동관리 > API 계정연결 > 연결하기 리젼 합치기
- [유져] 페이팔 API 연동하기 버튼 눌렀을때 즉시 되어야함, 키발급은 독립몰에서 사용하는 독립적인 것임
- [관리자] 사이드바 메뉴 위치 fixed 하기
- [관리자] 테이블의 모든 데이터 항목 전체 글씨 보이고 테이블에 row스크롤바 생기게 수정
- [유져] 페이고스 > 증빙현황 > 상세보기 > 플래폼지급ID 전체 ID 표기 되게 수정
- [선희님] 파일 전달
...
- [부장님] 사업자번호 로그인 API개발완료 > 프론트에 추가 할것
