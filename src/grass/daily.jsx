...
[11.19 수행예정]
#1. 로그인 로직 수정 => 로직 확정 된후 작업 예정 => 확인확인확인 해주세요
!! 18일 최종 정리로직으로는 가입을 페이고스로 시작했는데 디수정으로 시작했는지 알수 없다 => 이부분 괜찮은지 확인 필요
...
#2. 모든 테이블에서 FRRG_TS, LSMD_TS 삭제예정으로 금일중 모든 맵퍼 PAYGOS_SYS_FRRG_TS, PAYGOS_SYS_LSMD_TS참조하게 수정 
- 전체 중요 공지 전달: FRRG_TS, LSMD_TS 고치면서 유져 API의 PAYGOS_FRRG_ID, LSMD_ID 값 => PAYGOS-WEB으로 수정 하도록 전달

- 금일부터 UserBack도 loc사용

#########################################3
#FRRG_TD, LSMD_ID PAYGOS-WEB USER -> 통일
#########################################

[차순위 업무]
#3. Auth테스트(페이팔만, 아마존 유효기간 표시 갱신버튼, 프론트에서 업데이트 날짜+365일 해서 출력, Upsert사용을 연결,갱신 추가)
- [대표님 요청] exs001DB의 LSMD_TS+365일을 토큰 만료일로 화면에 표시하여 하단에 갱신하기버튼(연결하기 버튼과 동일동작)버튼 넣어주기)
- 연결해제 버튼 디자인 수정
#4. 약정신청할때 필수정보 요구하는지 확인해볼것
#5. 담당자 정보 Not Null로 변경 => 약정할떄 필요하다
#6. cmi002 LSMD_ID PAYGOS_ID 확인 PAYGOS-WEB인지 확인
#7. 테스트 계정, 테스트 DB에 전체 insertt
#4. 컨텐츠 디자인
...
- [선희님] 알러트 메시지 화면별 정리 요청건
    - 이책임님 상수 정의후 정리 예정
- [대표님] 이용안내 게시판 카테고리 추가 => 자주하는 질문 템플릿으로 변경
- [IBK요청] 파일첨부 PDF제한에서 모든 파일 허용, 50MG허용으로 변경 
- FRRG_ID,TS 사용 안할수도 있음-> 확정시 전체 코드 수정 => 대작업임
#5. 배너 > EX탭 추가, 화면에서 다수 배너 사라짐, 차라리 배너 히든 기능을 추가 하면 ?  

- T,X등급은 전체에 한명밖에 있을수 없음-> X=P T=Q로 변경
- [선희님] 파일 전달
...
- FRRG_ID,TS 사용 안할수도 있음-> 확정시 전체 코드 수정 => 대작업임
#1. 사번 중복 등록 처리 불가
#2. T,X등급은 전체에 한명밖에 있을수 없음-> X=P T=Q로 변경
#3.  X, T 등급은 각각 1명만 존재 가능 >>> 따라서 db에 기등록자가 있는 경우 신규 X, T 등급이 존재하면 안됨
#4. 메뉴상속..? => 요거는 몬지 모르겠눈뎀,,,,???b
#5. 현재 IP 수정 불가하여 수정가능하게 수정해야함 => 필수
- T,X등급은 전체에 한명밖에 있을수 없음-> X=P T=Q로 변경
- [선희님] 파일 전달
    - 사용자에게 전달되는 alert 메세지 목록화 작성
    - 랜딩 등과 같이 특정 상황에 대해 메세지 띄어지는 경우 확인     
