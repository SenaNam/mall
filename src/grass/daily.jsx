...
#04. 디수정 은행 페이지 내부에서 진행할때도 우리 002d DB에 서비스코드04인 데이터로 inset 하는지 확인 => 현재 insert하는듯?
...
- 연동중 상태 DB조회 로직으로 수정
#02. EAI에서 기존 약정정보 불러올때 서비스 코드 00으로 들어 오고 있는데 상황에 맞게 01->02 로 바까야함 -> 누가..?
    -> 002테이블 DB인서트이후 EAI호출시 EAI가 EAI값으로 덮어쓰는 문제였음 + 암복화문제 있음 
...
#07. 유져 > 약정 신청할때 이메일 이상하다고함 
#08. 서비스 약정상태코드 확인 후 등록상태만 화면에 보여줄것
 - 서비스, 마켓, 계좌변경
 - 독립몰 강제로 넣어둔것 모두 삭제할것(현재 계정계에느 URL ddd 뿐임 
#09. 어드민 >  페이지 새창 보기 사이즈 전부다 크게 수정 
#10. 유져 > 대시보드 독립몰 서비스 카드 로직 다시 생각, 서비스카드 컴포넌트 수정해야함
#13. 배너 요구사항 명세서 옴..음
#00. 유져 > 모바일랜딩화면에서 모달 사이즈 90vh 로 수정해줘야함
#00. [차장님] 유져 > 페이고스 > 대시보드 진입시 약정 refresh 해줄것
#00. [대표님] 기유져 로그인 형송이로 변경
...
[상기 처리 후 진행]
#17. 담당자 정보 Not Null로 변경 => 약정할때 필요하다?
