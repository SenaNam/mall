...
#00. [이랑님] 유져 > 약정 후 API 연결 국내/해외 페이지 이동 해외페이지 고정 수정 => 완료
#00. 유져 > 보호 라우터 추가 > GUEST로 세션 남아있는 현상 수정 => 완료
#01. [부장님] 유져 > 로그아웃 버튼 눌렀을때 우리쪽 로그아웃 프로세서 진행후 은행쪽 로그아웃 프로세서 진행을 위해 팝업 처리 => 완료
#00. 어드민 > loc.yml 파일 git ignore할것 => 완료
#00. 유져 > 페이팔 API연동시 화면 이동 건 수정 => 완료
#00. 유져 > 페이팔 API연동 결과 페이지 타이틀 추가 => 완료
#00. 유져 > 페이팔 API연동 결과 페이지 화면 수정 => 완료
#00. 유져 > 약관 ul > p태그 들어있는거 다시 수정 => 완료
#00. 유져 > 약정시 > 담당자 정보 없을때 대표 정보 가지고 가지 말고 그냥 에러 반환으로 수정 => 완료


[차순위 업무 > 상기 처리 후 진행 - 중요]
#00. 어드민 게시판 배포판 파일 등록 안됨 -> NAS경로 설정 에러 있음 => 수정완료 => 배포 하면 적용됨 내용 확인해야함
#03. [김우철과장님] 어드민 > 파일용량 확장 체크하여 전달 해주기로함 => 확장자 제한X, 50MB이하
#00. 유져 > 약정 페이지 허브전에 Hub3 만들어서 모든 담당자 정보 체크해서 null일경우 담당자 정보 필수입니다 alert후 담당자 정보 수정으로 locations => 보류
----------------------------------------------------------------------------------------------------------------------
#03. [이랑님] 어드민 > 배너 > 대기상태 -> DB랑 화면이랑 맞지 않음 확인 요청
#04. 페이고스 랜딩 배너 레이아웃 수정 =>  중배너, 대배너 수정해야함
#05. [대표님] 에러메세지 검토 수행사 수정 
#04. 배너 요구사항 명세서 옴..음 => 집에서 PPT파일 열었는데 안열림..캡쳐해서 전달하는걸로...확인...
#00. [김우철과장님] 페이팔 연동후 Email DB에 안들어옴 -_-)??, 연동중인데 미연동으로 뜸 => ...? , 완료
#01. 유져 > 자주 하는 질문 > 제목 수정 => 이랑님 전달 DB 이전후 작성 예정
...
- 구글 Auth 
- 컨텐츠 디자인
#05. 유져 > 페이팔 리설트 페이지에 코드랑 같이 오면 이미 등록된 계정정보입니다~~ 메세지 뜨게 => 배포 후 테스트 
#06. 암복화 인터셉터에서 통합 관리!
- 서비스 이용신청
...
- 서비스, 마켓, 계좌변경
- 독립몰 강제로 넣어둔것 모두 삭제할것(현재 계정계에느 URL ddd 뿐임)
