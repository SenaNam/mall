...
[11.20]
#1. DB FRRG_TS, LSMD_TS 컬럼 삭제건=> 부장님, 차장님 일괄 수정 => 완료
...
#2 API연동관리 > Auth 수정/테스트건
 - 페이팔 Auth테스트
 - [대표님] 아마존 유효기간 표시 갱신버튼, 프론트에서 업데이트 날짜+365일 해서 출력, 추가 => 완료
 - 연결해제 버튼 디자인 수정
#3. DB컬럼명 수정
...
#00. 어드민 > 권한관리
 - 사번 중복 등록 처리 불가
 - 현재 IP 수정 불가하여 수정가능하게 수정해야함 => 필수
 - T,X등급은 전체에 한명밖에 있을수 없음-> X=P T=Q로 변경
 - X, T 등급은 각각 1명만 존재 가능 >>> 따라서 db에 기등록자가 있는 경우 신규 X, T 등급이 존재하면 안됨
 - 메뉴상속? => 내용 확인 => 완료됬다함(선희님)
 - 권한관리 > T,X 권한 설명문 => 받음 > X 등급 (승인자), T등급 (자점감사자)
 - 만일 새로운 관리자를 X나 T로 등록하고 싶다면 >>> 기등록자의 등급을 바꿔야함

#00. [선희님] 사용자 메세지 문서화 파일 전달
 - 이책임님 상수 정의후 정리 예정
 - 사용자에게 전달되는 alert 메세지 목록화 작성
 - 랜딩 등과 같이 특정 상황에 대해 메세지 띄어지는 경우 확인     
 - 알러트및 수정, 확인등 전체

#00. [대표님] 유져 > 이용안내 게시판 수정 > 카테고리 추가 + 자주하는 질문 템플릿으로 변경

#00. [IBK요청] 어드민 > 파일첨부 PDF제한에서 모든 파일 허용, 50MG허용으로 변경 

#00. [이랑님] 어드민,유져> 배너 > EX탭 추가, 화면에서 다수 배너 사라짐 확인 필요
