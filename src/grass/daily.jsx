[11.20]
#00. DB FRRG_TS, LSMD_TS 컬럼 삭제건=> 부장님, 차장님 일괄 수정
...
#00. DB컬럼명 수정 => 이책임님 일괄 수정 예정
 - DA이슈로 다수의 DB컬럼명이 수정되어 맵퍼의 수정이 필요 하다고 함 
 => 현재 기존 컬럼과 암호화 컬럼 동시 존재
 => 이책임님이 일괄로 수정 후 각 개발 담당자가 테스트 확인 예정후 개별 수정예정
#00. API연동관리 > Auth 수정/테스트건
 - 페이팔 Auth테스트
 - [대표님] 아마존 유효기간 표시 갱신버튼, 프론트에서 업데이트 날짜+365일 해서 출력, 추가 => 완료
 - 연결해제 버튼 디자인 수정
#00. [대표님] 유져 > 이용안내 게시판 수정 > 카테고리 추가 + 자주하는 질문 템플릿으로 변경 => 완료
#00. [이랑님] 어드민,유져> 배너 > EX탭 추가, 화면에서 다수 배너 사라짐 확인 필요
#00. 컨텐츠 디자인
#00. [IBK요청] 어드민 > 파일첨부 PDF제한에서 모든 파일 허용, 50MG허용으로 변경 => 완료
#00. [이랑님] 어드민 > 게시판 > 수정 > 카테고리 변경 불가 => 완료
#00. 페이고스 랜딩 배너 레이아웃 수정 => 완료
