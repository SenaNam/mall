[28일 완료]
#00. [부장님] 유져 > 로그아웃 버튼 눌렀을때 우리쪽 로그아웃 프로세서 진행후 은행쪽 로그아웃 프로세서 진행을 위해 그쪽으로 리다이렉트 시켜줄것 => 1차 - 에러시 확인 후 바로 롤백 할것 => 롤백
#00. [차장님] 유져 > 페이팔 > 연동중인데 연결해제 버튼이 아니고 연결하기 버튼 출력중 수정 => 완료
#00. [김우철과장님] 아마존 콜백 페이지 > 마이서비스 링크 이전 페이지로 되어있는것 수정하기 => 완료
#00. [대표님] 유져 > 대시보드 > 서비스 시작하기 누르면 이동이 안됨 => 완료
#00. 유져 > 해외오픈마켓 서비스 > 아마존 페이지 제목 달기 => 완료
#00. 약관 브릿지에서 null을체크를 못해서 !Y로 변경해야함 => 빈값, null 모두 약관 N으로 처리함 => 확인 완료
#00. 어드민 > 배너 에약 게시 로직 설정, 배치나 스케쥴러 어떻게 돌려서 대기에 대한 상태 트리거 어떻게 할지 결정 => 어드민 배너 리스트 진입시 새로고침됨 => 완료
...
#08. 어드민 >  페이지 새창 보기 사이즈 전부다 크게 수정 => 완료페
#09. 배너 요구사항 명세서 옴..음 => 집에서 PPT파일 열었는데 안열림..캡쳐해서 전달하는걸로...확인...
#10. 유져 > 모바일랜딩화면에서 모달 사이즈 90vh 90vw 로 수정해줘야함 => 완료
...
3) 페이팔 연동 해제 기능 => 완료
5) 국내몰 약정 서비스 신청 => 완료
6) 로그아웃시 인뱅 로그아웃 처리 => ibk~로 리다이렉트 1차 => 롤백
7) 아마존 워딩 변경 => 대표님이 메신져에 워딩 줬다그랬는데 없음
...
#02. 유져 > 페이팔 화면에 필요 없는 요소 추가 체크 해주시면 지우겠음 => 대표님 확인 필요 -> 안해줌 -> 이미지줬다함
...
#04. 유져 > 대시보드 독립몰 서비스 카드 로직 다시 생각, 서비스카드 컴포넌트 수정해야함 => 완료
