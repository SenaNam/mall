...
#00. 유져/어드민 > 백엔드 > 파일다운로드 경로 상대경로로 수정
#05. 페이고스 랜딩 배너 레이아웃 수정 =>  중배너, 대배너 수정해야함
#00. 어드민 > 배너 디테일 화면에서 인풋 박스 전부 디스어블 처리
...
#00. 유져 > 파일다운 가능하게 수정
#00. 유져 > 배너에서 클릭되는 URL이 공지및 게시물일때 잘 이동 되는가
#00. 어드민 > 배너 > 순서 조절 이미지 출력
#00. 유져 > 공지사항 출력이 안됨 > 왜? > http: 보호라우터를 걸었기때문 => 이제됨 , 근데? 게시물당 :id출력 안됨
#00. 유져 로그인후 뒤로가기시 > "/" 로그아웃 => 엣지도 확인해야함
#00. 유져 > 대시보드 챠트 두개 너비 좁을때 겹침
#00. 유져 > 디수정 > 랜딩에서 SL배너 호출
#00. 어드민 > SL 배너 삭제 -> 디수정 랜딩 로깅용으로 사용 => 어드민 화면에서 삭제
#00. 유져 >. 가입시 문자 인증 수정해야함
#00. 어드민 > 세션관리 로컬스토리지로 이동 해야함
#00. 어드민 > 배너 순서 조정 이미지 보이기 수정 
#00. 유져 > 파일 다운로드 가능한지 배포버전에서 확인해야함
#00. 유져, 어드민 창닫았을때 로컬스토리지 어떻게 컨트롤할지 => 
#00. 어드민/유져 로그아웃 안하고 창닫았을때 방어로직 확인 
