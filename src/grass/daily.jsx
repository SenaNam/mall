[11.25(화) 작업일지]
[해결(이관)된 이슈]
#00. 익일 권한관리 수정 반영 확인후 페이고스방에 공지 (11.25) 
#00. 랜딩에 이용안내 추가 => 이랑님


[11.25(화) 금일 수행 업무] 
#00. cmi001m, cmi002d not null 컬럼 Not Null 로 변경된 건에 관하여 백엔드에 수정 반영 => 완료
#00. 디수정 > 로그인시 제출화면으로 이동 => 완료
#00. 디수정 > 랜딩에서 자세히 메뉴 없애기 => 완료
#00. [김우철과장님] CMI002D 약정테이블 넣는과정에 CSAC_IDNT_NO오류 => 신규 약관시 ''로 입력 => 완료
#00. 신규 가입시 이동페이지 수정 => 기존 api연동 (MyService페이지 없어짐 => 2개로 쪼개짐) => 완료
    - 페이고스, 디수정 이용안내 -> 이용안내 게시판으로 변경
    - 페이고스 바로 시작 -> 페이고스 대시보드
    - 디수정 바로 시작 -> 디수정 제출화면
#00. 독립몰 추가하기 기본 1개 보이게 기본 카드 수정 (로고 대신에 뭐 예쁜 샵 모양 ) => 완료 
#00. 유져 > 이용안내 아이콘 변경 => 완료
#00. [IBK요청] 유져 > 메뉴 통합 > 해외마켓/국내마켓 별 통합 메뉴
    - 해외 오픈마켓/ 국내개설 온라인몰 두가지 메뉴로 구성
    - 마켓별 API 연동 상태 관리 => 완료
    - 마켓별 계좌 연결 관리 => 완료
    - 페이팔 OAuth 연결 State 재확인
    - 화면 요소 재거 

    

#00. 로그인 로직 ver.6 (로그아웃 로직 수정)
    - 로그인 후 뒤로가기로 인해 발생할 수 있는 인증 재호출, 파라미터 노출, 온보딩 되돌림 등의 문제는 전부 제거
    - userType 또는 세션이 깨지는 경우에는 자동으로 로그아웃 후 초기 화면으로 이동하도록 통일
    - Hub1, 온보딩, 성공 페이지 등은 모두 뒤로가기가 차단되어 흐름이 일관되게 유지
    01. 로그인 직후 뒤로가기
    ...
    02. userType(intent) 누락된 상태에서 뒤로가기 -> 어제 대표님이 본 상황
...
#00. 페이고스 랜딩 배너 레이아웃 수정 =>  중배너, 대배너 수정해야함
#00. [김우철과장님] 어드민 > 파일용량 확장 체크하여 전달 해주기로함 
#00. [대표님] 에러메세지 검토 수행사 수정 
