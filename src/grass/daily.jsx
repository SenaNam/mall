[28일 완료]
#02. EAI에서 기존 약정정보 불러올때 서비스 코드 00으로 들어 오고 있는데 상황에 맞게 01->02 로 바까야함 -> 차장님이 완료
    -> 002테이블 DB인서트이후 EAI호출시 EAI가 EAI값으로 덮어쓰는 문제였음 + 암복화문제 있음 => 이책임님이 완료
#00. 유져 > 해외오픈마켓 > 아마존 OAuth 새탭으로 연결 Result페이지도 새탭으로 => 완료 -> 테스트 해야함
#07. 유져 > 계좌출력에서 이상한거 출력하고 있음 => 선희님이 해결 반영 후 확인 
#10. 유져 > 대시보드 독립몰 서비스 카드 로직 다시 생각, 서비스카드 컴포넌트 수정해야함 => 완료
#00. 유져 > 대시보드 서비스 카드 디자인 수정 > 개별출력 X , 통합출력or시작하기 => 완료
#17. 담당자 정보 Not Null로 변경 => 선희님이 002d에서 담당자 정보 가져가는것으로 => 선희님 수정 완료 
#00. [대표님] 기유져 로그인 형송이로 변경 => 완료
#00. [대표님] 기유져를 한타만전 => 완료
#00. [차장님] 유져 > 페이고스 > 대시보드 진입시 약정 refresh 해줄것 => 완료
...
#01. API연동관리 > 페이팔 
- 연동중 상태 DB조회 로직으로 수정
#03. 로그인 직후 뒤로가기 눌렀을때 허브에서 이전페이지여 상태를 체크해서 이전페이지가 온보딩일 경우 다시 해당하는 랜딩으로 보내주는 로직 추가 -> 테스트해야함
#04. [이랑님] 어드민 > 배너 > 대기상태 -> DB랑 화면이랑 맞지 않음 확인 요청
#05. 페이고스 랜딩 배너 레이아웃 수정 =>  중배너, 대배너 수정해야함
#06. [대표님] 에러메세지 검토 수행사 수정 
#08. 서비스 약정상태코드 확인 후 등록상태만 화면에 보여줄것
 - 서비스, 마켓, 계좌변경
 - 독립몰 강제로 넣어둔것 모두 삭제할것(현재 계정계에느 URL ddd 뿐임 
#09. 어드민 >  페이지 새창 보기 사이즈 전부다 크게 수정 
#13. 배너 요구사항 명세서 옴..음
#00. 유져 > 모바일랜딩화면에서 모달 사이즈 90vh 90vw 로 수정해줘야함
#00. [차장님] 유져 > 마켓 서비스 관리 > 현재 서비스 가입중인곳만 출력될수 있도록 "원장상태:1"
