...
#01. API연동관리 > 페이팔 
- 연동중 상태 DB조회 로직으로 수정
#02. 로그인 직후 뒤로가기 눌렀을때 허브에서 이전페이지여 상태를 체크해서 이전페이지가 온보딩일 경우 다시 해당하는 랜딩으로 보내주는 로직 추가 -> 테스트해야함
...
#08. 어드민 >  페이지 새창 보기 사이즈 전부다 크게 수정 
#09. 배너 요구사항 명세서 옴..음
#10. 유져 > 모바일랜딩화면에서 모달 사이즈 90vh 90vw 로 수정해줘야함



[차순위 업무 > 상기 처리 후 진행]
#00. [부장님] 유져 > 로그아웃 버튼 눌렀을때 우리쪽 로그아웃 프로세서 진행후 은행쪽 로그아웃 프로세서 진행을 위해 그쪽으로 리다이렉트 시켜줄것
#00. [차장님] 유져 > 페이팔 > 연동중인데 연결해제 버튼이 아니고 연결하기 버튼 출력중 수정 
#00. [김우철과장님] 아마존 콜백 페이지 > 마이서비스 링크 이전 페이지로 되어있는것 수정하기
#00. [김우철과장님] 페이팔 연동후 Email DB에 안들어옴 -_-)??, 연동중인데 미연동으로 뜸 
#00. 유져 > 해외오픈마켓 서비스 > 아마존 페이지 제목 달기 
#00. 약관 브릿지에서 null을체크를 못해서 !Y로 변경해야함e
...
#00. 어드민 > 배너 에약 게시 로직 설정, 배치나 스케쥴러 어떻게 돌려서 대기에 대한 상태 트리거 어떻게 할지 결정

[이슈 및 확인사항]
#00. 유져 > 독립몰 목록에서 약정이 살아 있는 목록만 뿌려 주면 된다
#00. 유져 로그인후 뒤로가기시 > "/" 로그아웃 
    - 엣지도 확인해야함
#00. 마이페이지 약정 후 > 프론팅 
- 약정신청할때 필수정보 요구하는지 확인해볼것
- 독립몰 약정신청시 등록된 사업자만 가능한거 어떻게 EAI에서 답변오는지 => URL기준으로 이미 중복판단????
- 금일부터 유져 정보 저장위치가 세션스토리지가에서 로컬스토리지로 변경(다른탭에서 여는 ex)OAuth이콜백 페이지 이슈등)
...
#08. EAI 에서 세션 새롭게 주입하는 곳중 어디에서 10자리 맞춰 주는지 확인 => 일단 완료로 퉁치기로 함      
#00. 유져 > 국내개설 온라인몰 서비스 관리 > 독립몰 조회 01 (둘다 String)
#07. 유져 > 약정 신청할때 이메일 이상하다고함 -> 먼가 수정된거 같음
