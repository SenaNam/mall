...
#00. 유져 >. 가입시 문자 인증 수정해야함 => 완료 어떤 요정님이 해주고 간거 같음
...
#00. 어드민 > 배너 출력 확인 => 배포 후 확인(백엔드)DRM 개색히
#00. 유젼 > 파일 다운로드 확인 => 배포 후 확인(백엔드) => DRM 개색히
#00. 유져 > 외환 가이드 분기 => ?
#00. 관리자 > 헤더에 GRD 출력 => No => API 리퀘스트헤드에 grd 추가해주기
#00. 뭐 고과장님이 사이드 배너 이벤트 배너 어쩌고? => 해결
#00. 어드민 > 사이드배너 사이즈 높이 80으로 변경됨 => 완료
#00. 유져 > 화이트리스트, 블랙리스트 전체 적용으로 확대할지 확인
#00. 유져 > 새요청시 토큰 재검증 하는지 부장님한테 확인
#00. 어드민 > SSO 로그인시 토큰 유효성 체크 후 에러 페이지 보낼때 REASON 부장님이 주기로함 AuthError페이지에서 확인후 처리
#00. 유져 > 로컬스토리지에 저장되는 Auth정보 로그아웃하지 않고 브라우저 닫을때는 어떻게 해야 하는가
#00. 이팔에 연동키를 입력해주세요 → 고객님! 페이고스와 연결할 키를 관리해주세요
#00. 고비즈코리아, 바이코리아는 메뉴에서 독립몰 키 메뉴 없애
#00. 독립몰은 연결하기/연결해제 → 연결관리로
#00. 들어가면 연결하기 연결해제가 있음
중요공지>
